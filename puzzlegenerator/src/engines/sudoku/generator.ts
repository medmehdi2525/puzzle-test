...
import { mirrorGrid, rotateGrid } from "@/engines/shared/latin";
...
export function generateSudoku(
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    let solution = generateFullGrid(gridSize, rng);
    const t = Math.floor(rng() * 4);
    if (t === 1) solution = rotateGrid(solution);
    else if (t === 2) solution = mirrorGrid(solution);
    else if (t === 3) solution = mirrorGrid(rotateGrid(solution));
